      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              {complaint?.location && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-secondary-900">{typeof complaint?.location === 'object' ? `${(complaint.location as any).lat}, ${(complaint.location as any).lng}` : complaint?.location}</p>
                  </div>
                </div>
              )}
          ...
          {complaint?.residentId && (
            <Card variant="md" className="p-6">
              ...
              <div className="space-y-4">
                {complaint?.residentId?.profile?.fullName && (
                  <div>
                    <label className="text-sm text-secondary-500">Name</label>
                    <p className="text-secondary-900">{complaint?.residentId?.profile?.fullName}</p>
                  </div>
                )}
                ...
                {(complaint?.residentId as any)?.profile?.phone && (
                  <div className="flex items-center gap-3">
                    <Phone className="w-5 h-5 text-secondary-400" />
                    <div>
                      <label className="text-sm text-secondary-500">Phone</label>
                      <p className="text-secondary-900">{(complaint?.residentId as any)?.profile?.phone}</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          {complaint?.attachedImages && complaint?.attachedImages?.length > 0 && (
            <Card variant="md" className="p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {complaint?.attachedImages?.map((attachment: any, index: number) => (
                  <a
                    key={index}
                    href={attachment}
                    target="_blank"
        ...
        <div className="space-y-6">
          ...
          <Card variant="md" className="p-6">
            ...
            {String(complaint?.status) !== 'resolved' && String(complaint?.status) !== 'closed' && String(complaint?.status) !== 'completed' ? (
              <div className="space-y-4">
                <div>
              ...
              <div className="mt-6 pt-6 border-t border-secondary-200">
                ...
                <div className="p-3 bg-primary-50 rounded-lg">
                  <p className="text-secondary-900 font-semibold">
                    {(complaint?.assignedStaffId as any)?.profile?.fullName || (complaint?.assignedStaffId as any)?.firstName + ' ' + ((complaint?.assignedStaffId as any)?.lastName || '')}
                  </p>
                  <p className="text-sm text-secondary-600">{(complaint?.assignedStaffId as any)?.position || (complaint?.assignedStaffId as any)?.role}</p>
                </div>
              </div>
            )}
          </Card>
